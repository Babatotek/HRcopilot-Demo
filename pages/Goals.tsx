...
import { DEMO_OBJECTIVES } from '../demoData';


const Goals: React.FC = () => {
  const [objectives, setObjectives] = useState<Objective[]>(DEMO_OBJECTIVES as Objective[]);
  ...
  const handleAiArchitect = async () => {
    ...
    try {
      ...
      const newObj: Objective = {
        ...
        ownerAvatar: 'https://picsum.photos/40/40?sig=me',
        category: 'Growth',
        status: 'ON_TRACK',
            ...
            <div className="space-y-1">
              {['All Goals', 'Company Strategic', 'Engineering Team', 'My Objectives'].map(f => (
                <button 
                  ...
                  className={`w-full text-left px-4 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${activeFilter === f ? 'bg-[#0047cc] text-white shadow-lg' : 'text-slate-500 hover:bg-white/5 hover:text-slate-300'}`}
                >
                  {f}
                </button>
              ))}
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="md:w-64 space-y-4">
                    <div className="flex items-center gap-3">
                       <div className="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 border border-white/10"><img src={obj.ownerAvatar} className="w-full h-full object-cover" alt={obj.ownerName} /></div>
                       <div>
