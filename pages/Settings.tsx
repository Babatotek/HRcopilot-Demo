...
const Settings: React.FC<SettingsProps> = ({ brand, onUpdate, userProfile, onUpdateProfile }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      <header>
        <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase italic">Platform <span className="text-[#0047cc]">Governance</span></h2>
      ...
      <div className="flex flex-wrap gap-8 border-b border-slate-200 dark:border-white/5 pb-0 overflow-x-auto">
        {tabs.map((tab) => (
          <button 
            key={tab.id}
            id={tab.id === 'brand' ? 'nav-settings-brand' : undefined}
            onClick={() => setActiveTab(tab.id as any)}
            className={`pb-4 text-[10px] font-black uppercase tracking-widest relative transition-all whitespace-nowrap flex items-center gap-2 ${activeTab === tab.id ? 'text-slate-900 dark:text-white' : 'text-slate-500 hover:text-slate-900 dark:hover:text-slate-300'}`}
          >
            ...
            <GlassCard title="Financial Localization" className="animate-in slide-in-from-bottom-2 duration-500 !p-10">
              <div className="space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  ...
                  <div className="p-8 bg-blue-500/5 border border-blue-500/10 rounded-[32px] flex gap-4">
                ...
                <div className="space-y-10">
                  <div className="flex items-center justify-between p-8 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-[32px]">
                    <div className="flex items-center gap-6">
                      <div className="w-16 h-16 rounded-[24px] bg-emerald-500/10 text-emerald-500 flex items-center justify-center text-3xl shadow-inner">☁️</div>
                      <div>
              ...
              <GlassCard className="border-rose-500/20 bg-rose-500/[0.02] !p-10">
                 <div className="space-y-10">
                    <div className="flex items-start gap-6">
                       <div className="w-16 h-16 rounded-[24px] bg-rose-500/10 text-rose-500 flex items-center justify-center text-3xl shadow-inner">⚠️</div>
                       <div>
          ...
          {activeTab === 'governance' && (
            <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-500">
              <div className="flex items-center gap-4 p-6 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-[32px] mb-6">
                <div className="w-12 h-12 rounded-2xl bg-indigo-500/10 text-indigo-500 flex items-center justify-center text-2xl shadow-inner">🏢</div>
                <div className="flex-1">
              ...
              <div className="p-6 bg-indigo-500/5 border border-indigo-500/10 rounded-[24px] flex gap-4 mb-8">
                <span className="text-indigo-500 text-xl">🌐</span>
                <p className="text-[11px] text-indigo-600 dark:text-indigo-400 leading-relaxed font-medium">Corporate Policy is the organisation-wide standard. It applies to every branch automatically — no branch configuration required. Set it once and it works everywhere.</p>
              </div>
