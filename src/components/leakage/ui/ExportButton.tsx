...
<head>
  <meta charset="UTF-8">
  <title>HR360+ Leakage Intelligence Report — ${profile.orgTypeLabel}</title>
  <style>
...
<body>
  ...
  <div class="footer">
    <p>Generated by HR360+ Organizational Intelligence Widget · All figures calculated from user-provided inputs against globally sourced benchmarks.</p>
...
export default function ExportButton({ results, profile, aiAnalysis, currency, locale }) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `HR360-Leakage-Report-${profile.orgTypeLabel?.replace(/\s+/g, '-') || 'Report'}-${new Date().toISOString().slice(0, 10)}.html`
